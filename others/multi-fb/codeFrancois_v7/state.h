...
// For 3x3: for each 32bits, 23bits are useless and  9bits are useful
...
// Compute the list of *all* next states
// The location of tile taken should be either X or empty
// Note: This function is not used in main.cpp
StateList computeNextStatesAll(State s) {
  StateList l;
  int k = 0;
  State ss = s << HALF; // Move the O tiles to the X positions so that we can check quickly if there is no O
  for(int i=0; i<NB_MOVE_PER_DIR; ++i) {
    if ((ss & MOVE_LEFT[i][0]) == 0) // There is no O
      l.state[k++] = (((s & MOVE_LEFT[i][3]) << 1) & MOVE_LEFT[i][3]) | ( s & ~MOVE_LEFT[i][3]) | MOVE_LEFT[i][2];
  }
  for(int i=0; i<NB_MOVE_PER_DIR; ++i) {
    if ((ss & MOVE_RIGHT[i][0]) == 0)
      l.state[k++] = (((s & MOVE_RIGHT[i][3]) >> 1) & MOVE_RIGHT[i][3]) | ( s & ~MOVE_RIGHT[i][3]) | MOVE_RIGHT[i][2];
  }
  for(int i=0; i<NB_MOVE_PER_DIR; ++i) {
    if ((ss & MOVE_UP[i][0]) == 0)
